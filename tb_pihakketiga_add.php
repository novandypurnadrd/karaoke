...
add_nocache_headers();
include("include/tb_pihakketiga_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tb_pihakketiga_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_pihakketiga_inline_add.htm";
else
	$templatefile = "tb_pihakketiga_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_pihakketiga_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing namainstansi - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_namainstansi = $pageObject->getControl("namainstansi", $id);
		$control_namainstansi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing namainstansi - end
//	processing jeniskeluar - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_jeniskeluar = $pageObject->getControl("jeniskeluar", $id);
		$control_jeniskeluar->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing jeniskeluar - end
...
//	processing namainstansi - start
			...
			if($inlineAddOption)
			{
				$control_namainstansi->afterSuccessfulSave();
			}
//	processing namainstansi - end
//	processing jeniskeluar - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_jeniskeluar->afterSuccessfulSave();
			}
//	processing jeniskeluar - end
//	processing amount - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_pihakketiga_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_pihakketiga_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_pihakketiga_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["namainstansi"]=@$avalues["namainstansi"];
	$defvalues["jeniskeluar"]=@$avalues["jeniskeluar"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("namainstansi"))
		$xt->assign("namainstansi_fieldblock",true);
	else
		$xt->assign("namainstansi_tabfieldblock",true);
	$xt->assign("namainstansi_label",true);
	if(isEnableSection508())
		$xt->assign_section("namainstansi_label","<label for=\"".GetInputElementId("namainstansi", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("jeniskeluar"))
		$xt->assign("jeniskeluar_fieldblock",true);
	else
		$xt->assign("jeniskeluar_tabfieldblock",true);
	$xt->assign("jeniskeluar_label",true);
	if(isEnableSection508())
		$xt->assign_section("jeniskeluar_label","<label for=\"".GetInputElementId("jeniskeluar", $id, PAGE_ADD)."\">","</label>");
...
//	namainstansi
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("namainstansi", $data, $keylink);
		$showValues["namainstansi"] = $value;
		$showFields[] = "namainstansi";
	}	
//	jeniskeluar
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("jeniskeluar", $data, $keylink);
		$showValues["jeniskeluar"] = $value;
		$showFields[] = "jeniskeluar";
	}	
//	amount
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	referensi
	...
	{	
		...
		$showRawValues["namainstansi"] = substr($data["namainstansi"],0,100);
		$showRawValues["jeniskeluar"] = substr($data["jeniskeluar"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tb_pihakketiga";
		$options['firstTime'] = 1;
