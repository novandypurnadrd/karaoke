...
$layout->blocks["top"][] = "details";$page_layouts["tb_sumbangan_edit"] = $layout;
...
$templatefile = "tb_sumbangan_edit.htm";
...
//	processing tglpay - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tglpay = $pageObject->getControl("tglpay", $id);
		$control_tglpay->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: tb_sumbangan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tb_sumbangan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tglpay_label","<label for=\"".GetInputElementId("tglpay", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tb_sumbangan_view.php?".$viewlink."'\"");
