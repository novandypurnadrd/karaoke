...
add_nocache_headers();
include("include/tb_piltugas_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tb_piltugas_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_piltugas_inline_add.htm";
else
	$templatefile = "tb_piltugas_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_piltugas_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing pilihan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pilihan = $pageObject->getControl("pilihan", $id);
		$control_pilihan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pilihan - end
...
//	processing pilihan - start
			...
			if($inlineAddOption)
			{
				$control_pilihan->afterSuccessfulSave();
			}
//	processing pilihan - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_piltugas_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_piltugas_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_piltugas_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["pilihan"]=@$avalues["pilihan"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("pilihan"))
		$xt->assign("pilihan_fieldblock",true);
	else
		$xt->assign("pilihan_tabfieldblock",true);
	$xt->assign("pilihan_label",true);
	if(isEnableSection508())
		$xt->assign_section("pilihan_label","<label for=\"".GetInputElementId("pilihan", $id, PAGE_ADD)."\">","</label>");
...
//	pilihan
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("pilihan", $data, $keylink);
		$showValues["pilihan"] = $value;
		$showFields[] = "pilihan";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["pilihan"] = substr($data["pilihan"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tb_piltugas";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="tb_tugas" && postvalue("field")=="forpay") 
	{
		$lookupFieldName = "forpay";
		$lookupTable = "tb_piltugas";
		$linkFieldName = "pilihan";
		$dispfield = "pilihan";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("tb_tugas", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "tb_tugas", "forpay");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("pilihan", $strTableName, true);
