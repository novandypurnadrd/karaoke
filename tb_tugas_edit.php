...
$layout->blocks["top"][] = "details";$page_layouts["tb_tugas_edit"] = $layout;
...
$templatefile = "tb_tugas_edit.htm";
...
//	processing nip - end
//	processing dept - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_dept = $pageObject->getControl("dept", $id);
		$control_dept->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing posisi - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_posisi = $pageObject->getControl("posisi", $id);
		$control_posisi->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing posisi - end
//	processing forpay - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_forpay = $pageObject->getControl("forpay", $id);
		$control_forpay->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: tb_tugas_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tb_tugas_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["posisi"] = $evalues["posisi"];
	$data["forpay"] = $evalues["forpay"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("dept_label","<label for=\"".GetInputElementId("dept", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("posisi_label","<label for=\"".GetInputElementId("posisi", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("forpay_label","<label for=\"".GetInputElementId("forpay", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tb_tugas_view.php?".$viewlink."'\"");
