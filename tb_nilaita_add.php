...
$layout->blocks["top"][] = "details";$page_layouts["tb_nilaita_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_nilaita_inline_add.htm";
else
	$templatefile = "tb_nilaita_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_nilaita_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing tujuan - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tujuan = $pageObject->getControl("tujuan", $id);
		$control_tujuan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tujuan - end
//	processing nilaita - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_nilaita = $pageObject->getControl("nilaita", $id);
		$control_nilaita->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nilaita - end
...
//	processing tujuan - start
			...
			if($inlineAddOption)
			{
				$control_tujuan->afterSuccessfulSave();
			}
//	processing tujuan - end
//	processing nilaita - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_nilaita->afterSuccessfulSave();
			}
//	processing nilaita - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_nilaita_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_nilaita_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_nilaita_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["tujuan"]=@$avalues["tujuan"];
	$defvalues["nilaita"]=@$avalues["nilaita"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("tujuan"))
		$xt->assign("tujuan_fieldblock",true);
	else
		$xt->assign("tujuan_tabfieldblock",true);
	$xt->assign("tujuan_label",true);
	if(isEnableSection508())
		$xt->assign_section("tujuan_label","<label for=\"".GetInputElementId("tujuan", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("nilaita"))
		$xt->assign("nilaita_fieldblock",true);
	else
		$xt->assign("nilaita_tabfieldblock",true);
	$xt->assign("nilaita_label",true);
	if(isEnableSection508())
		$xt->assign_section("nilaita_label","<label for=\"".GetInputElementId("nilaita", $id, PAGE_ADD)."\">","</label>");
...
//	tujuan
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("tujuan", $data, $keylink);
		$showValues["tujuan"] = $value;
		$showFields[] = "tujuan";
	}	
//	nilaita
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("nilaita", $data, $keylink);
		$showValues["nilaita"] = $value;
		$showFields[] = "nilaita";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["tujuan"] = substr($data["tujuan"],0,100);
		$showRawValues["nilaita"] = substr($data["nilaita"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tb_nilaita";
		$options['firstTime'] = 1;
