...
add_nocache_headers();
include("include/tb_medical_variables.php");
...
$layout = new TLayout("add2","BoldWhite_label","MobileWhite_label");
...
$layout->blocks["top"][] = "details";$page_layouts["tb_medical_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_medical_inline_add.htm";
else
	$templatefile = "tb_medical_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_medical_add.php');
...
//	processing dept - start
	...
	if($inlineAddOption)
	{
		$control_dept = $pageObject->getControl("dept", $id);
		$control_dept->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing dept - end
//	processing section - start
	...
	if($inlineAddOption)
	{
		...
		$control_section->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing section - end
//	processing posisi - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_posisi = $pageObject->getControl("posisi", $id);
		$control_posisi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing posisi - end
//	processing payfor - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_payfor = $pageObject->getControl("payfor", $id);
		$control_payfor->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing payfor - end
//	processing namapayfor - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_namapayfor = $pageObject->getControl("namapayfor", $id);
		$control_namapayfor->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing namapayfor - end
...
//	processing tglbayar - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tglbayar = $pageObject->getControl("tglbayar", $id);
		$control_tglbayar->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tglbayar - end
//	processing referensi - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_referensi = $pageObject->getControl("referensi", $id);
		$control_referensi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing referensi - end
...
//	processing dept - start
			...
			if($inlineAddOption)
			{
				$control_dept->afterSuccessfulSave();
			}
//	processing dept - end
//	processing section - start
			$inlineAddOption = true;
...
//	processing posisi - start
			...
			if($inlineAddOption)
			{
				$control_posisi->afterSuccessfulSave();
			}
//	processing posisi - end
//	processing payfor - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_payfor->afterSuccessfulSave();
			}
//	processing payfor - end
//	processing namapayfor - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_namapayfor->afterSuccessfulSave();
			}
//	processing namapayfor - end
//	processing amount - start
			$inlineAddOption = true;
...
//	processing tglbayar - start
			...
			if($inlineAddOption)
			{
				$control_tglbayar->afterSuccessfulSave();
			}
//	processing tglbayar - end
//	processing referensi - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_referensi->afterSuccessfulSave();
			}
//	processing referensi - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_medical_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_medical_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_medical_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["section"]=@$avalues["section"];
	$defvalues["posisi"]=@$avalues["posisi"];
	$defvalues["payfor"]=@$avalues["payfor"];
	$defvalues["amount"]=@$avalues["amount"];
	$defvalues["tglbayar"]=@$avalues["tglbayar"];
	$defvalues["referensi"]=@$avalues["referensi"];
	$defvalues["nama"]=@$avalues["nama"];
	$defvalues["namapayfor"]=@$avalues["namapayfor"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("dept_label","<label for=\"".GetInputElementId("dept", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("section_label","<label for=\"".GetInputElementId("section", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("posisi_label","<label for=\"".GetInputElementId("posisi", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("payfor"))
		$xt->assign("payfor_fieldblock",true);
	else
		$xt->assign("payfor_tabfieldblock",true);
	$xt->assign("payfor_label",true);
	if(isEnableSection508())
		$xt->assign_section("payfor_label","<label for=\"".GetInputElementId("payfor", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("tglbayar"))
		$xt->assign("tglbayar_fieldblock",true);
	else
		$xt->assign("tglbayar_tabfieldblock",true);
	$xt->assign("tglbayar_label",true);
	if(isEnableSection508())
		$xt->assign_section("tglbayar_label","<label for=\"".GetInputElementId("tglbayar", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("referensi_label","<label for=\"".GetInputElementId("referensi", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("namapayfor"))
		$xt->assign("namapayfor_fieldblock",true);
	else
		$xt->assign("namapayfor_tabfieldblock",true);
	$xt->assign("namapayfor_label",true);
	if(isEnableSection508())
		$xt->assign_section("namapayfor_label","<label for=\"".GetInputElementId("namapayfor", $id, PAGE_ADD)."\">","</label>");
...
//	payfor
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("payfor", $data, $keylink);
		$showValues["payfor"] = $value;
		$showFields[] = "payfor";
	}	
//	amount
	$display = false;
...
//	tglbayar
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("tglbayar", $data, $keylink);
		$showValues["tglbayar"] = $value;
		$showFields[] = "tglbayar";
	}	
//	referensi
	$display = false;
	...
	{	
		...
		$showValues["referensi"] = $value;
		$showFields[] = "referensi";
	}	
//	nama
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	namapayfor
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("namapayfor", $data, $keylink);
		$showValues["namapayfor"] = $value;
		$showFields[] = "namapayfor";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["nip"] = substr($data["nip"],0,100);
		$showRawValues["dept"] = substr($data["dept"],0,100);
		$showRawValues["section"] = substr($data["section"],0,100);
		$showRawValues["posisi"] = substr($data["posisi"],0,100);
		$showRawValues["payfor"] = substr($data["payfor"],0,100);
		$showRawValues["amount"] = substr($data["amount"],0,100);
		$showRawValues["tglbayar"] = substr($data["tglbayar"],0,100);
		$showRawValues["referensi"] = substr($data["referensi"],0,100);
		$showRawValues["nama"] = substr($data["nama"],0,100);
		$showRawValues["namapayfor"] = substr($data["namapayfor"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tb_medical";
		$options['firstTime'] = 1;
