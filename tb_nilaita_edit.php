...
$layout->blocks["top"][] = "details";$page_layouts["tb_nilaita_edit"] = $layout;
...
$templatefile = "tb_nilaita_edit.htm";
...
//	processing tujuan - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tujuan = $pageObject->getControl("tujuan", $id);
		$control_tujuan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tujuan - end
//	processing nilaita - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_nilaita = $pageObject->getControl("nilaita", $id);
		$control_nilaita->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: tb_nilaita_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tb_nilaita_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["tujuan"] = $evalues["tujuan"];
	$data["nilaita"] = $evalues["nilaita"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tujuan_label","<label for=\"".GetInputElementId("tujuan", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nilaita_label","<label for=\"".GetInputElementId("nilaita", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tb_nilaita_view.php?".$viewlink."'\"");
