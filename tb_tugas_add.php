...
add_nocache_headers();
include("include/tb_tugas_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tb_tugas_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_tugas_inline_add.htm";
else
	$templatefile = "tb_tugas_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_tugas_add.php');
...
//	processing forpay - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_forpay = $pageObject->getControl("forpay", $id);
		$control_forpay->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing forpay - end
...
//	processing forpay - start
			...
			if($inlineAddOption)
			{
				$control_forpay->afterSuccessfulSave();
			}
//	processing forpay - end
//	processing amount - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_tugas_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_tugas_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_tugas_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["forpay"]=@$avalues["forpay"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("forpay_label",true);
	if(isEnableSection508())
		$xt->assign_section("forpay_label","<label for=\"".GetInputElementId("forpay", $id, PAGE_ADD)."\">","</label>");
...
//	nama
	...
	{	
		...
		$showRawValues["forpay"] = substr($data["forpay"],0,100);
