...
$layout->blocks["top"][] = "details";$page_layouts["tb_pihakketiga_edit"] = $layout;
...
$templatefile = "tb_pihakketiga_edit.htm";
...
//	processing namainstansi - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_namainstansi = $pageObject->getControl("namainstansi", $id);
		$control_namainstansi->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing namainstansi - end
//	processing jeniskeluar - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_jeniskeluar = $pageObject->getControl("jeniskeluar", $id);
		$control_jeniskeluar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing tglbayar - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tglbayar = $pageObject->getControl("tglbayar", $id);
		$control_tglbayar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tglbayar - end
//	processing referensi - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_referensi = $pageObject->getControl("referensi", $id);
		$control_referensi->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: tb_pihakketiga_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tb_pihakketiga_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["namainstansi"] = $evalues["namainstansi"];
	$data["jeniskeluar"] = $evalues["jeniskeluar"];
	$data["amount"] = $evalues["amount"];
	$data["tglbayar"] = $evalues["tglbayar"];
	$data["referensi"] = $evalues["referensi"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("namainstansi_label","<label for=\"".GetInputElementId("namainstansi", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jeniskeluar_label","<label for=\"".GetInputElementId("jeniskeluar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tglbayar_label","<label for=\"".GetInputElementId("tglbayar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("referensi_label","<label for=\"".GetInputElementId("referensi", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tb_pihakketiga_view.php?".$viewlink."'\"");
