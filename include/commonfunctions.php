...
function checkTableName($shortTName, $type=false)
{
	...
	if ("tb_tugas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_ta" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_sumbangan" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_reportpihak3" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_report" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_medical" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_meal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_pihakketiga" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_piltugas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tb_nilaita" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetUserPermissions($table="")
{
	...
	if($table=="tb_sumbangan")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="tb_reportpihak3")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="tb_report")
	...
	if($table=="tb_pihakketiga")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="tb_piltugas")
	{
			$permissions =  "ADEPIM";
	}
	if($table=="tb_nilaita")
...
function inFacebook(){
	...
	if ($signed_request) {
		...
		$strSQL = "select * from ".AddTableWrappers("tb_user")." where ".AddFieldWrappers(GetUserNameField())."='".$strUsername."'";
...
function AfterFBLogIn($pUsername, $pPassword){
	...
	$strSQL = "select * from ".AddTableWrappers("tb_user")." where ".AddFieldWrappers($cUserNameField)."=".$strUsername."";
	$rs = db_query($strSQL,$conn);
 	$data = db_fetch_array($rs);
	if($data){
		$logged=true;
