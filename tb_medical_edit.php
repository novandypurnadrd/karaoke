...
$layout->blocks["top"][] = "details";$page_layouts["tb_medical_edit"] = $layout;
...
$templatefile = "tb_medical_edit.htm";
...
//	processing payfor - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_payfor = $pageObject->getControl("payfor", $id);
		$control_payfor->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing nama - end
//	processing namapayfor - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_namapayfor = $pageObject->getControl("namapayfor", $id);
		$control_namapayfor->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: tb_medical_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("payfor_label","<label for=\"".GetInputElementId("payfor", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("namapayfor_label","<label for=\"".GetInputElementId("namapayfor", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tb_medical_view.php?".$viewlink."'\"");
