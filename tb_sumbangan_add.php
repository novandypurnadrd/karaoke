...
add_nocache_headers();
include("include/tb_sumbangan_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tb_sumbangan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tb_sumbangan_inline_add.htm";
else
	$templatefile = "tb_sumbangan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tb_sumbangan_add.php');
...
//	processing tglpay - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tglpay = $pageObject->getControl("tglpay", $id);
		$control_tglpay->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tglpay - end
...
//	processing tglpay - start
			...
			if($inlineAddOption)
			{
				$control_tglpay->afterSuccessfulSave();
			}
//	processing tglpay - end
//	processing referensi - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tb_sumbangan_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tb_sumbangan_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tb_sumbangan_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["tglpay"]=@$avalues["tglpay"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tglpay_label","<label for=\"".GetInputElementId("tglpay", $id, PAGE_ADD)."\">","</label>");
...
//	referensi
	...
	{	
		...
		$showRawValues["tglpay"] = substr($data["tglpay"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tb_sumbangan";
		$options['firstTime'] = 1;
